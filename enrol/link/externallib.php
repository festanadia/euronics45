...
/**
 * link enrol plugin external functions
 *
 * @package    enrol_link
...
/**
 * link enrolment external functions.
 *
 * @package   enrol_link
 ...
 * @since     Moodle 2.6
 */
class enrol_link_external extends external_api {
    ...
    public static function get_instance_info_parameters() {
        return new external_function_parameters(
                array('instanceid' => new external_value(PARAM_INT, 'instance id of link enrolment plugin.'))
            );
    }

    /**
     * Return link-enrolment instance information.
     *
     * @param int $instanceid instance id of link enrolment plugin.
    ...
    public static function get_instance_info($instanceid) {
        ...
        // Retrieve link enrolment plugin.
        $enrolplugin = enrol_get_plugin('link');
    ...
    public static function enrol_user_parameters() {
        return new external_function_parameters(
            array(
                ...
                'instanceid' => new external_value(PARAM_INT, 'Instance id of link enrolment plugin.', VALUE_DEFAULT, 0)
            )
        );
    }

    /**
     * link enrol the current user in the given course.
     *
    ...
    public static function enrol_user($courseid, $password = '', $instanceid = 0) {
        ...
        // Retrieve the link enrolment plugin.
        $enrol = enrol_get_plugin('link');
        if (empty($enrol)) {
            throw new moodle_exception('canntenrol', 'enrol_link');
        }

        // We can expect multiple link-enrolment instances.
        $instances = array();
        $enrolinstances = enrol_get_instances($course->id, true);
        foreach ($enrolinstances as $courseenrolinstance) {
            if ($courseenrolinstance->enrol == "link") {
                // Instance specified.
        ...
        foreach ($instances as $instance) {
            $enrolstatus = $enrol->can_link_enrol($instance);
            if ($enrolstatus === true) {
                if ($instance->password and $params['password'] !== $instance->password) {
                    ...
                    if ($instance->customint1) {
                        ...
                        if (!enrol_link_check_group_enrolment_key($course->id, $params['password'])) {
                            $warnings[] = array(
                                'item' => 'instance',
                                'itemid' => $instance->id,
                                'warningcode' => '2',
                                'message' => get_string('passwordinvalid', 'enrol_link')
                            );
                            continue;
                        }
                    } else {
                        if ($enrol->get_config('showhint')) {
                            ...
                            $warnings[] = array(
                                ...
                                'itemid' => $instance->id,
                                'warningcode' => '3',
                                'message' => s(get_string('passwordinvalidhint', 'enrol_link', $hint)) // message is PARAM_TEXT.
                            );
                            continue;
                        } else {
                            $warnings[] = array(
                ...
                $enrol->enrol_link($instance, (object) $data);
                $enrolled = true;
                break;
            } else {
                $warnings[] = array(
