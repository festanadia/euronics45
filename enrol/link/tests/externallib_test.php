...
/**
 * link enrol external PHPunit tests
 *
 * @package   enrol_link
...
require_once($CFG->dirroot . '/enrol/link/externallib.php');

class enrol_link_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_get_instance_info() {
        ...
        // Check if link enrolment plugin is enabled.
        $linkplugin = enrol_get_plugin('link');
        $this->assertNotEmpty($linkplugin);
        ...
        $instanceid1 = $linkplugin->add_instance($course, array('status' => ENROL_INSTANCE_ENABLED,
                                                                'name' => 'Test instance 1',
                                                                'customint6' => 1,
                                                                'roleid' => $studentrole->id));
        $instanceid2 = $linkplugin->add_instance($course, array('status' => ENROL_INSTANCE_DISABLED,
                                                                'customint6' => 1,
                                                                'name' => 'Test instance 2',
                                                                'roleid' => $studentrole->id));

        $instanceid3 = $linkplugin->add_instance($course, array('status' => ENROL_INSTANCE_ENABLED,
                                                                'roleid' => $studentrole->id,
                                                                'customint6' => 1,
        ...
        $this->setAdminUser();
        $instanceinfo1 = enrol_link_external::get_instance_info($instanceid1);
        $instanceinfo1 = external_api::clean_returnvalue(enrol_link_external::get_instance_info_returns(), $instanceinfo1);

        $this->assertEquals($instanceid1, $instanceinfo1['id']);
        $this->assertEquals($course->id, $instanceinfo1['courseid']);
        $this->assertEquals('link', $instanceinfo1['type']);
        ...
        $instanceinfo2 = enrol_link_external::get_instance_info($instanceid2);
        $instanceinfo2 = external_api::clean_returnvalue(enrol_link_external::get_instance_info_returns(), $instanceinfo2);
        $this->assertEquals($instanceid2, $instanceinfo2['id']);
        $this->assertEquals($course->id, $instanceinfo2['courseid']);
        $this->assertEquals('link', $instanceinfo2['type']);
        $this->assertEquals('Test instance 2', $instanceinfo2['name']);
        $this->assertEquals(get_string('canntenrol', 'enrol_link'), $instanceinfo2['status']);
        $this->assertFalse(isset($instanceinfo2['enrolpassword']));

        $instanceinfo3 = enrol_link_external::get_instance_info($instanceid3);
        $instanceinfo3 = external_api::clean_returnvalue(enrol_link_external::get_instance_info_returns(), $instanceinfo3);
        $this->assertEquals($instanceid3, $instanceinfo3['id']);
        $this->assertEquals($course->id, $instanceinfo3['courseid']);
        $this->assertEquals('link', $instanceinfo3['type']);
        $this->assertEquals('Test instance 3', $instanceinfo3['name']);
        $this->assertTrue($instanceinfo3['status']);
        $this->assertEquals(get_string('password', 'enrol_link'), $instanceinfo3['enrolpassword']);

        // Try to retrieve information using a normal user for a hidden course.
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        try {
            enrol_link_external::get_instance_info($instanceid3);
    ...
    public function test_enrol_user() {
        ...
        $linkplugin = enrol_get_plugin('link');
        $studentrole = $DB->get_record('role', array('shortname' => 'student'));
        $instance1id = $linkplugin->add_instance($course1, array('status' => ENROL_INSTANCE_ENABLED,
                                                                'name' => 'Test instance 1',
                                                                'customint6' => 1,
                                                                'roleid' => $studentrole->id));
        $instance2id = $linkplugin->add_instance($course2, array('status' => ENROL_INSTANCE_DISABLED,
                                                                'customint6' => 1,
                                                                'name' => 'Test instance 2',
                                                                'roleid' => $studentrole->id));
        ...
        // link enrol me.
        $result = enrol_link_external::enrol_user($course1->id);
        $result = external_api::clean_returnvalue(enrol_link_external::enrol_user_returns(), $result);
        ...
        try {
            enrol_link_external::enrol_user($course2->id);
        } catch (moodle_exception $e) {
            self::assertEquals('canntenrol', $e->errorcode);
        }

        // Enable the instance.
        $linkplugin->update_status($instance2, ENROL_INSTANCE_ENABLED);

        // Try not passing a key.
        $result = enrol_link_external::enrol_user($course2->id);
        $result = external_api::clean_returnvalue(enrol_link_external::enrol_user_returns(), $result);
        ...
        $result = enrol_link_external::enrol_user($course2->id, 'invalidkey');
        $result = external_api::clean_returnvalue(enrol_link_external::enrol_user_returns(), $result);
        self::assertFalse($result['status']);
        self::assertCount(1, $result['warnings']);
        self::assertEquals('4', $result['warnings'][0]['warningcode']);
        ...
        $linkplugin->set_config('showhint', true);
        $result = enrol_link_external::enrol_user($course2->id, 'invalidkey');
        $result = external_api::clean_returnvalue(enrol_link_external::enrol_user_returns(), $result);
        self::assertFalse($result['status']);
        self::assertCount(1, $result['warnings']);
        ...
        $result = enrol_link_external::enrol_user($course2->id, 'abcdef');
        $result = external_api::clean_returnvalue(enrol_link_external::enrol_user_returns(), $result);
        ...
        // Now, everything ok.
        $result = enrol_link_external::enrol_user($course2->id, 'zyx');
        $result = external_api::clean_returnvalue(enrol_link_external::enrol_user_returns(), $result);
        ...
        $instance3id = $linkplugin->add_instance($course2, array('status' => ENROL_INSTANCE_ENABLED,
                                                                'customint6' => 1,
                                                                'name' => 'Test instance 2',
                                                                'roleid' => $studentrole->id));
        ...
        self::setUser($user4);
        $result = enrol_link_external::enrol_user($course2->id, 'abcdef', $instance3id);
        $result = external_api::clean_returnvalue(enrol_link_external::enrol_user_returns(), $result);
        self::assertTrue($result['status']);
        self::assertTrue(is_enrolled($context2, $user3));
