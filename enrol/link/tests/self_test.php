...
global $CFG;
require_once($CFG->dirroot.'/enrol/link/lib.php');
require_once($CFG->dirroot.'/enrol/link/locallib.php');

class enrol_link_testcase extends advanced_testcase {

    public function test_basics() {
        $this->assertTrue(enrol_is_enabled('link'));
        $plugin = enrol_get_plugin('link');
        $this->assertInstanceOf('enrol_link_plugin', $plugin);
        $this->assertEquals(1, get_config('enrol_link', 'defaultenrol'));
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, get_config('enrol_link', 'expiredaction'));
    }

    public function test_sync_nothing() {
        global $SITE;

        $selfplugin = enrol_get_plugin('link');
    ...
    public function test_longtimnosee() {
        ...
        $this->assertEquals(3, $DB->count_records('enrol', array('enrol'=>'link')));
        $instance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'link'), '*', MUST_EXIST);
        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'link'), '*', MUST_EXIST);
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'link'), '*', MUST_EXIST);
    ...
    public function test_send_expiry_notifications() {
        ...
        /** @var $selfplugin enrol_link_plugin */
        $selfplugin = enrol_get_plugin('link');
        ...
        $this->assertEquals(4, $DB->count_records('enrol', array('enrol'=>'link')));
        ...
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'link'), '*', MUST_EXIST);
    ...
    public function test_show_enrolme_link() {
        ...
        // Cohort member test.
        $instance5 = $DB->get_record('enrol', array('courseid'=>$course5->id, 'enrol'=>'link'), '*', MUST_EXIST);
        ...
        $instance7 = $DB->get_record('enrol', array('courseid'=>$course6->id, 'enrol'=>'link'), '*', MUST_EXIST);
        ...
        $instance8 = $DB->get_record('enrol', array('courseid'=>$course7->id, 'enrol'=>'link'), '*', MUST_EXIST);
        ...
        $instance9 = $DB->get_record('enrol', array('courseid'=>$course8->id, 'enrol'=>'link'), '*', MUST_EXIST);
        ...
        $instance10 = $DB->get_record('enrol', array('courseid'=>$course9->id, 'enrol'=>'link'), '*', MUST_EXIST);
        ...
        $instance11 = $DB->get_record('enrol', array('courseid'=>$course10->id, 'enrol'=>'link'), '*', MUST_EXIST);
        ...
        $instance12 = $DB->get_record('enrol', array('courseid'=>$course11->id, 'enrol'=>'link'), '*', MUST_EXIST);
     ...
     */
    public function test_can_link_enrol() {
        ...
        $expectederrorstring = get_string('canntenrol', 'enrol_link');
        ...
        $this->setUser($guest);
        $this->assertStringContainsString(get_string('noguestaccess', 'link'),
                $selfplugin->can_link_enrol($instance1, true));

        $this->setUser($user1);
        $this->assertTrue($selfplugin->can_link_enrol($instance1, true));

        // Active enroled user.
        $this->setUser($user2);
        $selfplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $this->setUser($user1);
        $this->assertSame($expectederrorstring, $selfplugin->can_link_enrol($instance1, true));
    }

    /**
     * Test enrol_link_check_group_enrolment_key
     */
    public function test_enrol_link_check_group_enrolment_key() {
        global $DB;
        ...
        $result = enrol_link_check_group_enrolment_key($course->id, 'invalidpassword');
        $this->assertFalse($result);

        $result = enrol_link_check_group_enrolment_key($course->id, 'thepassword');
        ...
        $result = enrol_link_check_group_enrolment_key($othercourse->id, 'thepassword');
        $this->assertFalse($result);
    ...
    public function test_get_welcome_email_contact() {
        ...
        // Enable link enrolment plugin and set to send email from course contact.
        $selfplugin = enrol_get_plugin('link');
        $instance1 = $DB->get_record('enrol', ['courseid' => $course1->id, 'enrol' => 'link'], '*', MUST_EXIST);
        $instance1->customint6 = 1;
        ...
        assign_capability('enrol/link:holdkey', CAP_ALLOW, $managerrole->id, $context);
    ...
    public function test_get_user_enrolment_actions() {
        ...
        // Only enable the link enrol plugin.
