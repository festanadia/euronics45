...
/**
 * link enrolment plugin implementation.
 * @author Petr Skoda
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_link_plugin extends enrol_plugin {
    ...
    public function get_info_icons(array $instances) {
        ...
        foreach ($instances as $instance) {
            if ($this->can_link_enrol($instance, false) !== true) {
                ...
                // would hide link-enrolment icons from guests.
                continue;
            }
        ...
        $icons = array();
        if ($nokey) {
            $icons[] = new pix_icon('withoutkey', get_string('pluginname', 'enrol_link'), 'enrol_link');
        }
        if ($key) {
            $icons[] = new pix_icon('withkey', get_string('pluginname', 'enrol_link'), 'enrol_link');
        }
        return $icons;
    }
    ...
    public function show_enrolme_link(stdClass $instance) {
        ...
        if (true !== $this->can_link_enrol($instance, false)) {
            return false;
        }
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/link:config', $context)) {
            return false;
        }
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        require_once("$CFG->dirroot/enrol/link/locallib.php");

        $enrolstatus = $this->can_link_enrol($instance);

        if (true === $enrolstatus) {
            // This user can link enrol using this instance.
            $form = new enrol_link_enrol_form(null, $instance);
            $instanceid = optional_param('instance', 0, PARAM_INT);
            if ($instance->id == $instanceid) {
                if ($data = $form->get_data()) {
                    $this->enrol_link($instance, $data);
                }
            }
        } else {
            // This user can not link enrol using this instance. Using an empty form to keep
            // the UI consistent with other enrolment plugins that returns a form.
            $data = new stdClass();
            $data->header = $this->get_instance_name($instance);
            $data->info = $enrolstatus;

            // The can_link_enrol call returns a button to the login page if the user is a
            // guest, setting the login url to the form if that is the case.
            $url = isguestuser() ? get_login_url() : null;
            $form = new enrol_link_empty_form($url, $data);
        }
     ...
     */
    public function can_link_enrol(stdClass $instance, $checkuserenrolment = true) {
        ...
        if ($checkuserenrolment) {
            ...
            if ($DB->get_record('user_enrolments', array('userid' => $USER->id, 'enrolid' => $instance->id))) {
                return get_string('canntenrol', 'enrol_link');
            }
        }
        ...
        if (!has_capability('enrol/link:enrollink', context_course::instance($instance->courseid))) {
            return get_string('canntenrol', 'enrol_link');
        }

        if ($instance->enrolstartdate != 0 and $instance->enrolstartdate > time()) {
            return get_string('canntenrolearly', 'enrol_link', userdate($instance->enrolstartdate));
        }

        if ($instance->enrolenddate != 0 and $instance->enrolenddate < time()) {
            return get_string('canntenrollate', 'enrol_link', userdate($instance->enrolenddate));
        }
        ...
        if ($instance->customint3 > 0) {
            ...
            if ($count >= $instance->customint3) {
                // Bad luck, no more link enrolments here.
                return get_string('maxenrolledreached', 'enrol_link');
            }
        }
        ...
        if ($instance->customint5) {
            ...
            if (!cohort_is_member($instance->customint5, $USER->id)) {
                ...
                return markdown_to_html(get_string('cohortnonmemberinfo', 'enrol_link', $a));
            }
        }
    ...
    public function get_enrol_info(stdClass $instance) {
        ...
        $instanceinfo->status = $this->can_link_enrol($instance);

        if ($instance->password) {
            $instanceinfo->requiredparam = new stdClass();
            $instanceinfo->requiredparam->enrolpassword = get_string('password', 'enrol_link');
        }
        ...
        if ((true === $instanceinfo->status) && $instance->password) {
            $instanceinfo->wsfunction = 'enrol_link_get_instance_info';
        }
        return $instanceinfo;
    }
    ...
    protected function email_welcome_message($instance, $user) {
        ...
        if (trim($instance->customtext1) !== '') {
            ...
            $messagetext = get_string('welcometocoursetext', 'enrol_link', $a);
            $messagehtml = text_to_html($messagetext, null, false, true);
        }

        $subject = get_string('welcometocourse', 'enrol_link', format_string($course->fullname, true, array('context'=>$context)));
    ...
    public function sync(progress_trace $trace, $courseid = null) {
        ...
        $trace->output('Verifying link-enrolments...');
        ...
        // Note: the logic of link enrolment guarantees that user logged in at least once (=== u.lastaccess set)
        ...
        $sql = "SELECT e.*, ue.userid
                  ...
                  JOIN {enrol} e ON (e.id = ue.enrolid AND e.enrol = 'link' AND e.customint2 > 0)
        ...
        $trace->output('...user link-enrolment updates finished.');
        $trace->finished();
    ...
    /**
     * Returns the user who is responsible for link enrolments in given instance.
     *
     * Usually it is the first editing teacher - the person with "highest authority"
     * as defined by sort_by_roleassignment_authority() having 'enrol/link:manage'
     * capability.
     *
    ...
    protected function get_enroller($instanceid) {
        ...
        if ($users = get_enrolled_users($context, 'enrol/link:manage')) {
        ...
        if ($merge and $instances = $DB->get_records('enrol', $merge, 'id')) {
            ...
            if (!empty($data->customint5)) {
                if ($step->get_task()->is_samesite()) {
                    ...
                    // Use some id that can not exist in order to prevent link enrolment,
    ...
    public function restore_role_assignment($instance, $roleid, $userid, $contextid) {
        ...
        // we do not use component in manual or link enrol.
    ...
    public function can_delete_instance($instance) {
        $context = context_course::instance($instance->courseid);
        return has_capability('enrol/link:config', $context);
    }
    ...
    public function can_hide_show_instance($instance) {
        ...
        if (!has_capability('enrol/link:config', $context)) {
            return false;
        }
    ...
    protected function get_expirynotify_options() {
        $options = array(0 => get_string('no'),
                         1 => get_string('expirynotifyenroller', 'enrol_link'),
                         2 => get_string('expirynotifyall', 'enrol_link'));
        return $options;
    }
    ...
    /**
     * The link enrollment plugin has several bulk operations that can be performed.
    ...
    public function get_bulk_operations(course_enrolment_manager $manager) {
        ...
        if (has_capability("enrol/link:manage", $context)) {
            $bulkoperations['editselectedusers'] = new enrol_link_editselectedusers_operation($manager, $this);
        }
        if (has_capability("enrol/link:unenrol", $context)) {
            $bulkoperations['deleteselectedusers'] = new enrol_link_deleteselectedusers_operation($manager, $this);
        }
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $context) {
        ...
        $mform->addElement('select', 'status', get_string('status', 'enrol_link'), $options);
        $mform->addHelpButton('status', 'status', 'enrol_link');

        $options = $this->get_newenrols_options();
        $mform->addElement('select', 'customint6', get_string('newenrols', 'enrol_link'), $options);
        $mform->addHelpButton('customint6', 'newenrols', 'enrol_link');
        $mform->disabledIf('customint6', 'status', 'eq', ENROL_INSTANCE_DISABLED);

        $passattribs = array('size' => '20', 'maxlength' => '50');
        $mform->addElement('passwordunmask', 'password', get_string('password', 'enrol_link'), $passattribs);
        $mform->addHelpButton('password', 'password', 'enrol_link');
        ...
        $mform->addElement('select', 'customint1', get_string('groupkey', 'enrol_link'), $options);
        $mform->addHelpButton('customint1', 'groupkey', 'enrol_link');

        $roles = $this->extend_assignable_roles($context, $instance->roleid);
        $mform->addElement('select', 'roleid', get_string('role', 'enrol_link'), $roles);

        $options = array('optional' => true, 'defaultunit' => 86400);
        $mform->addElement('duration', 'enrolperiod', get_string('enrolperiod', 'enrol_link'), $options);
        $mform->addHelpButton('enrolperiod', 'enrolperiod', 'enrol_link');
        ...
        $mform->addElement('date_time_selector', 'enrolstartdate', get_string('enrolstartdate', 'enrol_link'), $options);
        $mform->setDefault('enrolstartdate', 0);
        $mform->addHelpButton('enrolstartdate', 'enrolstartdate', 'enrol_link');

        $options = array('optional' => true);
        $mform->addElement('date_time_selector', 'enrolenddate', get_string('enrolenddate', 'enrol_link'), $options);
        $mform->setDefault('enrolenddate', 0);
        $mform->addHelpButton('enrolenddate', 'enrolenddate', 'enrol_link');

        $options = $this->get_longtimenosee_options();
        $mform->addElement('select', 'customint2', get_string('longtimenosee', 'enrol_link'), $options);
        $mform->addHelpButton('customint2', 'longtimenosee', 'enrol_link');

        $mform->addElement('text', 'customint3', get_string('maxenrolled', 'enrol_link'));
        $mform->addHelpButton('customint3', 'maxenrolled', 'enrol_link');
        ...
        if (count($cohorts) > 1) {
            $mform->addElement('select', 'customint5', get_string('cohortonly', 'enrol_link'), $cohorts);
            $mform->addHelpButton('customint5', 'cohortonly', 'enrol_link');
        } else {
        ...
        $mform->addElement('select', 'customint4', get_string('sendcoursewelcomemessage', 'enrol_link'),
                enrol_send_welcome_email_options());
        $mform->addHelpButton('customint4', 'sendcoursewelcomemessage', 'enrol_link');

        $options = array('cols' => '60', 'rows' => '8');
        $mform->addElement('textarea', 'customtext1', get_string('customwelcomemessage', 'enrol_link'), $options);
        $mform->addHelpButton('customtext1', 'customwelcomemessage', 'enrol_link');

        if (enrol_accessing_via_instance($instance)) {
            $warntext = get_string('instanceeditlinkwarningtext', 'core_enrol');
            $mform->addElement('static', 'linkwarn', get_string('instanceeditlinkwarning', 'core_enrol'), $warntext);
        }
    }
    ...
    public function edit_instance_validation($data, $files, $instance, $context) {
        ...
        if ($data['status'] == ENROL_INSTANCE_ENABLED) {
            if (!empty($data['enrolenddate']) and $data['enrolenddate'] < $data['enrolstartdate']) {
                $errors['enrolenddate'] = get_string('enrolenddaterror', 'enrol_link');
            }
        }
    ...
    /**
     * Gets a list of roles that this user can assign for the course as the default for link-enrolment.
     *
     * @param context $context the context.
     * @param integer $defaultrole the id of the role that is set as the default for link-enrolment
    ...
    public function get_welcome_email_contact($sendoption, $context) {
        ...
        if ($sendoption == ENROL_SEND_EMAIL_FROM_COURSE_CONTACT) {
            ...
            // Send as the first user with enrol/link:holdkey capability assigned in the course.
            list($sort) = users_order_by_sql('u');
            $keyholders = get_users_by_capability($context, 'enrol/link:holdkey', 'u.*', $sort);
            if (!empty($keyholders)) {
 ...
 */
function enrol_link_get_fontawesome_icon_map() {
    return [
        'enrol_link:withkey' => 'fa-key',
        'enrol_link:withoutkey' => 'fa-sign-in',
    ];
}
