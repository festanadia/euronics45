...
class custom_menu extends custom_menu_item {
    ...
    public static function convert_text_to_menu_nodes($text, $language = null) {
        ...
        /* Ariadne 2025 */
	if (!self::has_capability_gestione_formazione()) return $root->get_children();
   ...
   /**  Ariadne2025
     ...
     * il menÃ¹ Gestione. Gli utenti che possono visualizzarlo sono l'admin, respo_hr,
    ...
    **/
    static function has_capability_gestione_formazione() {
        global $DB;
        global $USER;
		
		if (is_siteadmin($USER)) return true;
        $log = false;
        $array_roles = ['respo_hr', 'store_manager'];

        try {
            foreach ($array_roles as $roles_of_check) {
                // ottieni l'id del corso 'respo_hr'
