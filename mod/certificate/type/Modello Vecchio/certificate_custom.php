...
certificate_print_text($pdf, $x+5, $y+45, 'C', 'Helvetica', 'B', 22, strtoupper(fullname($customUser)),$txtareawdth);
//certificate_print_text($pdf, $x+5, $y+155, 'C', 'Helvetica', 'B', 18, date('d/m/Y', time()),$txtareawdth);

$pdf->SetTextColor(255, 255, 255);

$result = getEuronicsPersonalAttribute($customUser,$certificate->scormname,'euronics');

if(isset($customUser->azpiva)) {
    $infofooter = $customUser->azdenominazione.' - P.IVA:'.$customUser->azpiva.' Cap '.$customUser->azcap.' - '.$customUser->azcitta.' ('.$customUser->azprovincia.')';
    certificate_print_text($pdf, $x, $y+220, 'C', 'Helvetica', '', 10, $infofooter,$txtareawdth);
    certificate_print_text($pdf, $x+12, $y-20, 'L', 'Helvetica', 'B', 8, $customUser->azdenominazione,$txtareawdth);
}
