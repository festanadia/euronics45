...
require_once("$CFG->dirroot/mod/certificate/locallib_itd.php");
...
if (empty($action)) { // Not displaying PDF
    ...
    if ($attempts = certificate_get_attempts($certificate->id)) {
     //NF - rimossa la tabella dei tentativi
    ...
    echo "<p>Inserisci id utenti per generare il certificato</p><textarea id='elenco'></textarea><button id='genera'>Genera certificati</button><p id='risp'></p>";
    ...
    $(()=>{
      ...
      function generaCertificati(){
          var elenco = $("#elenco").val().trim();

        if(elenco.length==0){
            $("#risp").html("Inserisci gli id separati da ,");
            return;
        }
 
        $("#risp").html("<p>Attendere prego stiamo generando i certificati</p>");
        $.ajax({
            url: location.href+"&action=get",
            type:"POST",
            data:{
                elenco:elenco
            },
            success:(risp)=>{
                ...
                var risp = JSON.parse(risp);
                $("#risp").html("<p>Certificati generati :"+ risp.generati +"</p>")
                $("#risp").append("<p>Non generati:"+ risp.error.join(",")+" </p>")
            }

        })

      }
      $("#genera").click(generaCertificati);  
    ...
    //$elenco = optional_param('elenco', '', PARAM_ALPHA);
    // No debugging here, sorry.
   
   
    $elenco = explode(",", $_POST["elenco"]);
    $error =[];
    foreach($elenco as $idUser){
        try{
            $customUser = getUser($idUser);
            $certrecord = certificate_get_issue($course, $idUser, $certificate, $cm);
            require("$CFG->dirroot/mod/certificate/type/$certificate->certificatetype/certificate_custom.php");
            ...
            if ($certificate->savecert == 1) {
                certificate_save_pdf($filecontents, $certrecord->id, $filename, $context->id,$idUser);
            }
