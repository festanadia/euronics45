...
function certificate_get_issues($certificateid, $sort="ci.timecreated ASC", $groupmode, $cm, $page = 0, $perpage = 0) {
    ...
    $picturefields = 'u.id'; //user_picture::fields('u', get_extra_user_fields($context));
    $users = $DB->get_records_sql("SELECT u.firstname,u.lastname,u.institution,u.department, u.idnumber, ci.code, ci.timecreated
                                     FROM {user} u
...
function getEuronicsPersonalAttribute($user, $sconame, $dbstatistics = null) {
    global $DB;

    $user->nome = strtoupper($user->firstname);
    $user->cognome = strtoupper($user->lastname);
    $user->codicefiscale = $user->idnumber;
    if(strlen($user->codicefiscale)==16) {
        $user->birthyear = (string) substr($user->codicefiscale, 6, 2);
        $birtday = intval(substr($user->codicefiscale, 9, 2));
        $gender = 'M';
        if ($birtday > 40) {
            $birtday = $birtday - 40;
            $gender = 'F';
        ...
        $user->gender = $gender;
        if ($birtday < 10) {
            $user->birthday = '0' . $birtday;
        } else {
            $user->birthday = $birtday;
        }

        $tempbirthmonth = substr($user->codicefiscale, 8, 1);
        switch ($tempbirthmonth) {
            ...
            $user->birthmonth = $tempbirmonth;
            $idnazionale = substr($user->codicefiscale, 11, 4);
            $resultlocation = $DB->get_record('comuni', array('id_nazionale' => $idnazionale));
            if ($resultlocation) {
                $user->residenza = $resultlocation->comune;
                $user->provincia = $resultlocation->provincia;
            }
    }
    $sql = "SELECT MIN(t.Timeitem) AS timestart,MAX(t.Timeitem) as timend FROM (SELECT sc.timemodified AS Timeitem FROM mdl_scorm_scoes_track sc
                    JOIN mdl_user us ON
                    us.id = sc.userid
                    JOIN mdl_scorm s ON
                    s.id = sc.scormid
                WHERE us.username = '" . $user->username . "'
					and s.course = (SELECT course FROM mdl_scorm  WHERE name = '" . $sconame . "')
                    order by sc.timemodified) t
                ";
    $result = $DB->get_record_sql($sql);
    if ($result) {
        $user->coursestart = date('d/m/Y', $result->timestart);
        $user->courseend = date('d/m/Y', $result->timend);
    }

    $sql = "SELECT piva,mandante,cap,citta,provincia
                    FROM
                      mdl_piva_soci
                    WHERE
	...
	$sql = "SELECT CodiceAteco as codateco, Mansione as profiloprofessionale
                    FROM
                      mdl_codici_ateco
                    WHERE
                      lower(username) = '" . strtolower($user->username) . "'
    ...
    $result1 = $DB->get_record_sql($sql);
	
	$sql = "select count(*) as isalea from mdl_user where username='" . strtolower($user->username) . "' and upper(department) like '% ALEA %'";
	$result2 = $DB->get_record_sql($sql);
	
	
    if ($result) 
	{
                $user->azpiva = $result->piva;
				if ($result2->isalea && $result2->isalea>0)
				{
					$user->azdenominazione = "ALEA S.R.L.";
				} else {
					$user->azdenominazione = $result->mandante;
				}
				$user->codateco =$result1->codateco;
                $user->profiloprofessionale = $result1->profiloprofessionale;
                $user->azcap = $result->cap;
                $user->azcitta =  $result->citta;
                $user->azprovincia = $result->provincia;      
    } else {
                $user->azpiva = "";
                $user->azdenominazione = "";
				$user->codateco ="";
                $user->profiloprofessionale = "";
                $user->azcap = "";
                $user->azcitta =  "";
                $user->azprovincia = "";  	
	}
...
function getEuronicsPersonalAttributeRischAgg($user, $courseid, $dbstatistics = null) {
    global $DB;
	...
	$sql = "select MIN(sc.timemodified) AS timestart, MAX(sc.timemodified) as timend 
			from {course_modules} cm
			inner join {scorm_scoes_track_massima} sc on cm.instance=sc.scormid
			inner join {user} u on sc.userid=u.id
			where cm.course=". $courseid . " and cm.module=19 and u.username= '" . $user->username . "' ";
    ...
    if ($result) 
	{
				...
				$user->codateco =$result->codateco;
                $user->profiloprofessionale = $result->profiloprofessionale;
                $user->azcap = $result->cap;
                $user->azcitta =  $result->citta;
                $user->azprovincia = $result->provincia;      
    } else {
