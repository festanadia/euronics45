...
$extrafields = array(); //\core_user\fields::for_identity($context, false);
...
$table->head = array($strto,'Azienda Socia','Punto Vendita');
$table->align = array('left');
foreach ($extrafields as $field) {
    $table->head[] = get_user_field_name($field);
    $table->align[] = 'left';
}
$table->head = array_merge($table->head, array($strdate, $strcode));
...
foreach ($users as $user) {
    ...
    $datainstitution[] = $user->institution;
    $datadepartment = array();
    $datadepartment[] = $user->department;
	
    foreach ($extrafields as $field) {
        $data[] = $user->$field;
    }
    //$data = array_merge($data, $datainstitution,$datadepartment, array($date, certificate_get_grade($certificate, $course, $user->id), $code));
	$data = array_merge($data, $datainstitution,$datadepartment, array($date,  $code));
    $table->data[] = $data;
}
