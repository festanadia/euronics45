...
defined('MOODLE_INTERNAL') || die();
  ...
  */
function xmldb_block_aelrecentlyaccessedcourses_install() {
    global $DB;

    if ($DB->count_records('block_instances') < 1) {
        // Only add the aelrecentlyaccessedcourses block if it's being installed on an existing site.
    ...
    $page->blocks->add_block('aelrecentlyaccessedcourses', 'content', 0, false, 'my-index', $subpagepattern);
    ...
    // aelrecentlyaccessedcourses block without the admin resetting all of the dashboards.
    //
    // Instead we'll just add the aelrecentlyaccessedcourses block to their dashboards here.
    ...
    foreach ($existingrecords as $existingrecord) {
        ...
        if (isset($seencontexts[$parentcontextid])) {
            ...
            // to add duplicate aelrecentlyaccessedcourses blocks to the same context. This happens
        ...
        $blockinstances[] = [
            'blockname' => 'aelrecentlyaccessedcourses',
