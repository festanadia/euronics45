...
defined('MOODLE_INTERNAL') || die();
  ...
  */
function xmldb_block_aelchosenbyyou_install() {
    global $DB;

    if ($DB->count_records('block_instances') < 1) {
        // Only add the aelchosenbyyou block if it's being installed on an existing site.
    ...
    $page->blocks->add_block('aelchosenbyyou', 'content', 0, false, 'my-index', $subpagepattern);
    ...
    // aelchosenbyyou block without the admin resetting all of the dashboards.
    //
    // Instead we'll just add the aelchosenbyyou block to their dashboards here.
    ...
    foreach ($existingrecords as $existingrecord) {
        ...
        if (isset($seencontexts[$parentcontextid])) {
            ...
            // to add duplicate aelchosenbyyou blocks to the same context. This happens
